...
#### This game is made for the Code Institute third project, a Python terminal game. It is designed to play a quiz game about the Planets in our solar system! It could suite any user interested in quiz games or the solar system. The quiz contains 8 questions with a, b or c answers. The user is asked to enter username and answer the questions and then the scores display and the user also gets the opportunity to print the top 5 scores to see if the result is in there. 
...
* Tested that the data was gotten as expected from the google sheet "planets-quiz" by printing out data from the spreadsheet and also adding data from run.py to the spreadsheet. The data was added and printed correctly as expected.
...
#### No unfixed bugs.
#### When printing the top 5 scores unfortunately the row might be altered a bit to the right if the username is too long, but I found no better way to print out (this was the most visualy appealing and clear way I found) and in most cases the username is not to long to fit.

![Issue printing top 5 scores](assets/images/issue-print-scores.png)
...
* At first the questions and options (at first only containing 8 quesitions/options) were printed out in the same order each time (the way they were stored). In order to add a shuffle feature (and also alter between 14 questions) the planet_qustions dict needed to be zipped with options list in order to keep the right answers together with the right options (and to solv the problem that the planet_questions was a dict and the options a list), but in order to use random.shuffle the zip was converted to a list item, zipped_list. After shuffle and only leave 8 items in the list (to only print out 8 questions each game) the zipped_list was converted back to a dictionary, d. Now the old questions were still printed out instead of the shuffled ones since they were printed from the planet_questions dict and options list.

![Bug 2](assets/images/bug2_1.png)

In order to fix this the items() was used to iterate keys and values through the new, shuffled dictionary, d, to print out the shuffled questions.

![Bug 2](assets/images/bug2_2.png)
...
#### Inspiration and help has come from the Code Institute projects [Love Sandwiches](https://github.com/Karasp1980/love_sandwiches).The answers of the questions has been checked using [Wikipedia](https://www.wikipedia.org) and [Stora boken om rymden](https://www.adlibris.com/se/bok/stora-boken-om-rymden-9789180182232) 

#### The following sites have also been very helpful:
